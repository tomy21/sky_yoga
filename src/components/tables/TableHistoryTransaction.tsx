...
import { useTransactionHistory } from "@/hooks/useHistoryTransaction";
import { HistoryTrxWithUser } from "@/app/api/controller/historyTransactionController";
import { format } from "date-fns";

export default function TableHistoryTransaction() {
  ...
  const transactionHistory: HistoryTrxWithUser[] = data?.data || [];
            ...
            <Table>
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                {isLoading ? (
                  ...
                  transactionHistory.map((items, index) => (
                    <TableRow key={items.id}>
                      ...
                      <TableCell className="text-theme-sm px-4 py-3 text-start text-gray-500 dark:text-gray-400">
                        {items.createdAt
                          ? format(new Date(items.createdAt), "dd MMM yyyy")
                          : "-"}
                      </TableCell>
                      <TableCell className="text-theme-sm px-4 py-3 text-start text-gray-500 dark:text-gray-400">
                        {items.trxNo}
                      ...
                      <TableCell className="text-theme-sm px-4 py-3 text-start text-gray-500 dark:text-gray-400">
                        {items.amount.toLocaleString("id-ID", {
                          style: "currency",
                          currency: "IDR",
